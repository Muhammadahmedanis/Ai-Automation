    ...
    const fetchData = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                if(param.name){
                    const filtered = updatedLeads.filter(lead => 
                        lead.Company && lead.Company.toLowerCase() === param.name.toLowerCase()
                    );
    ...
    <div className="p-4 border-b">
      ...
      {param.name && (
        <div className="mt-2 text-sm text-gray-600">
          Showing contacts from: <span className="font-medium">{param.name}</span>
          {displayPeople.length > 0 && (
            <span className="ml-2">({displayPeople.length} contacts)</span>
          )}
        </div>
      )}
    </div>
          ...
          {displayPeople.length === 0 ? (
            <tr>
              <td colSpan={7} className="text-center text-gray-400 py-8">
                {param.name 
                  ? `No contacts found for company: ${param.name}`
                  : "No contacts found"
                }
              </td>
            </tr>
